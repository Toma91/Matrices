...
func test_a() -> Double {
    ...
    let ptr1: [[Float]] = Array(repeating: Array(repeating: 0, count: PPPPPPPP), count: PPPPPPPP)
    let ptr2: [[Float]] = Array(repeating: Array(repeating: 0, count: PPPPPPPP), count: PPPPPPPP)
    var result: [[Float]] = Array(repeating: Array(repeating: 0, count: PPPPPPPP), count: PPPPPPPP)
...
@inline(never)
func test_b() -> Double {
    let ptr1 = UnsafeMutablePointer<Float>.allocate(capacity: PPPPPPPP * PPPPPPPP)
    let ptr2 = UnsafeMutablePointer<Float>.allocate(capacity: PPPPPPPP * PPPPPPPP)
    var result = UnsafeMutablePointer<Float>.allocate(capacity: PPPPPPPP * PPPPPPPP)
    
    defer { ptr1.deallocate(capacity: PPPPPPPP * PPPPPPPP) }
    defer { ptr2.deallocate(capacity: PPPPPPPP * PPPPPPPP) }
    defer { result.deallocate(capacity: PPPPPPPP * PPPPPPPP) }
    ...
    vDSP_mmul(
        ...
        result, 1,
        vDSP_Length(PPPPPPPP), vDSP_Length(PPPPPPPP), vDSP_Length(PPPPPPPP)
    )
    let d2 = Date()
...
func test_c() -> Double {
    ...
    let m1 = Matrix<Float>(nRows: PPPPPPPP, nColumns: PPPPPPPP)
    let m2 = Matrix<Float>(nRows: PPPPPPPP, nColumns: PPPPPPPP)
