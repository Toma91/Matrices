...
@_inlineable
public func /<T: BinaryFloatingPoint>(lhs: ColumnVector<T>, rhs: T) -> ColumnVector<T> {
...
@_inlineable
public func /<T: BinaryFloatingPoint>(lhs: RowVector<T>, rhs: T) -> RowVector<T> {
    let storage = Storage<T>(size: lhs.length)
    let m = lhs.length % 4
