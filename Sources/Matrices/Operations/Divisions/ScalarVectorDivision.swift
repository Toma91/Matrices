...
@_inlineable
public func /<T: BinaryFloatingPoint>(lhs: T, rhs: ColumnVector<T>) -> ColumnVector<T> {
...
@_inlineable
public func /<T: BinaryFloatingPoint>(lhs: T, rhs: RowVector<T>) -> RowVector<T> {
    let storage = Storage<T>(size: rhs.length)
    let m = rhs.length % 4
